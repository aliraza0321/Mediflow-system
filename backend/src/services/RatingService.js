...
class RatingService {
  ...
  submit(payload, patientId) {
    ensureRequiredFields(payload, ["doctorId", "appointmentId", "rating"]);

    const appointment = this.appointmentRepository.findById(payload.appointmentId);
    if (!appointment) {
    ...
    if (appointment.rated || this.ratingRepository.findByAppointmentId(payload.appointmentId)) {
    ...
    const rating = new DoctorRating({
      id: this.database.nextId("rating"),
