...
function Home() {
  ...
  const submitFeedback = async () => {
    ...
    if (role !== "patient") {
      alert("Feedback can be sent from patient accounts.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/support", {
        ...
        body: JSON.stringify({
          message: `Feedback from ${feedback.name || "Patient"}: ${feedback.message}`,
        }),
      });
  ...
  return (
    <>
      ...
      <section className="mb-14 flex flex-col items-center">
        <h2 className="text-2xl font-bold mb-6 text-blue-700">Services</h2>
